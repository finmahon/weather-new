...
function buoyRoute() {
  ...
  buoyRoute.get('/', function (req, res, next) {
    console.log(new Date(), 'In dublinBuoy route POST / req.body=', req.body);


    JSDOM.fromURL("https://twitter.com/DublinBayBuoy", {})
      .then(dom => {
        ...
        if (dom) {
          ...
          console.log('error from Buoy ')
          next({ error: 'error fetching Buoy data' });
        }
      })
      .catch(err => {
        console.log('error from get Buoy data', err)
        next({ error: 'error fetching Buoy data' });
      });;
